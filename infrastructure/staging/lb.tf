...
resource "aws_lb_listener" "alb_backend_api_listener" {
  ...
  depends_on = [aws_alb.alb_backend_api]
}

resource "aws_alb_listener" "alb_backend_api_listener_https" {
  load_balancer_arn = aws_alb.alb_backend_api.arn
  port              = "443"
  protocol          = "HTTPS"
  ...
  depends_on = [aws_alb.alb_backend_api, aws_acm_certificate_validation.lb_cert]
}